...
const CONFIG = {
  ...
  /* --- CHANGED: BLOOM DISABLED --- */
  bloomStrength: 0,   // Was 1.5. Setting to 0 removes the glow.
  bloomRadius: 0,     // Was 0.4
  bloomThreshold: 0,
...
/* --- CHANGED: DISABLED BLOOM PASS --- */
// We leave the code here but commented out or inactive so performance is better
// without calculating the glow.
...
/* Note: LineBasicMaterial will now look like simple flat lines */
...
function tick() {
  ...
  /* REMOVED: Bloom trigger intensity logic */
  /* The core light can stay as a flat light source, but we lower intensity slightly */
  let bloomFactor = 0;
  if (logoReveal > 0.7) {
    bloomFactor = (logoReveal - 0.7) / 0.3;
    bloomFactor = Math.min(bloomFactor, 1);
  }
  coreLight.intensity = bloomFactor * 5 * fadeOut; // Reduced from 10 to 5
